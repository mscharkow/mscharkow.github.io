...
-   team work (in small teams) is more fun and highly encouraged
...
- early computational work in the late 1960s (Annenberg Conference, General Inquirer)
- early simulation studies and formal modeling in the 1970s (Schelling's Segregation model)
- first AI hype in communication in the 1980s (Amsterdam School)
- @lazerpentland2009 position piece on computational *social* science in general
- ICA interest group and division *computational methods* in 2010s
- first professorships in Mainz, Vienna, Ilmenau, Munich
...
-   the "Big 3" approaches in CCS:
    ...
    3.  simulation and agent-based modeling
-   historically, *very* strong emphasis on web data and automatic content analysis [@vanatteveldt2022computational]
-   agent-based modeling is still a very niche topic (see your own study choices for next week!)
...
-   the field is moving extremely fast, that means
    ...
    -   platforms restrict access to data and prohibit the distribution of replication data
...
## CCS Paper Potpourri (in 2 weeks)

- choose from a sample of 25 articles, from *classics* to very recent papers
- create posters for the article, highlighting RQ, methods, findings
-   10 posters overall, we'll probably do 2x 30 min sessions 
-   remember to get the posters printed ASAP at the ZDV (A2 or A1 normal paper)
-   please bring the digital versions as well, just in case
-   any questions about the studies or posters?

## Next week

- first practical exercises for warming up
- bring your laptop or use the ZDV desktop
- we cover infrastructure/workflow issues
...
* Solutions: *Literate Programming* - mixing text and code



## Knitr and Quarto

*   the knitr package integrates R code directly into your writing
*   it automates result inclusion, saving time and reducing errors
*   Quarto uses pandoc for versatile document conversion, i.e. markdown to html/pdf/docx/epub, etc.
...
*   quarto uses markdown as its base language for text
*   **common examples:**
    *   `**bold**` $\rightarrow$ **bold**
    *   `*italic*` $\rightarrow$ *italic*
    *   `# Heading` $\rightarrow$ structural headings
    *   `- List` $\rightarrow$ bullet points
    *   `[Link](url)` $\rightarrow$ hyperlinks
    *   `![Image](path)` $\rightarrow$ images

## The YAML Header

*   The configuration block at the top of the file (between `---` and `---`)
*   Defines document-wide settings and metadata
*   **Examples:**
    *   **Metadata:** `title: "Analysis"`, `author: "Student"`, `bibliography: refs.bib`
    *   **Output Options:** `format: html`, `number-sections: true`, `toc: true`
    *   **Visuals:** `mainfont: "Times New Roman"`, `theme: minimalist`
*   Detailed visual options can be moved to a separate `_brand.yml` file


## Knitr code chunk example
...
![https://kieranhealy.org/resources/](images/workflow-wide-quarto.png)
...
## Scientific writing and references

- RStudio provides a visual mode for editing
- edit document with buttons and menus, useful for citations, images and tables
...
- similarly, there is a [Zotero connector](https://www.zotero.org/download/) if you already use Zotero's online library
...
*   IfP Quarto website: <https://code.ifp.uni-mainz.de/IfP/ma-datenanalyse>
...
-   Is informed consent needed for a content analysis of social media posts from members of parliament?
...
-   collab model (e.g. Meta): you can work with the platforms on specific research questions
...
-   mostly only public **content** data with little metadata, e.g. time stamps and account information
...
-   logs can be sent to an online server, or downloaded and donated manually
...
2.  How can we read multiple files? `list.files()` & `map()` functions

## Session detection

1.  Describe the basic algorithm for identifying user sessions.
...
a = '[{"quote":"If that’s true, if you don’t know who I am, then maybe your best course… would be to tread lightly.","author":"Walter White"},{"quote":"Sometimes it just feels better not to talk. At All. About Anything. To Anyone.","author":"Walter White"}]'
a |> jsonlite::prettify()
```
...
-   LMMs learn to represent different modalities in a shared "embedding space," understanding how text relates to visuals
...
-   in this course, we use LLM/LMM for essentially all content analysis tasks
...
-   LLM API requests
-   structured JSON responses and types
...
-   get single tasks to work first, then run the same task repeatedly using `map()` etc.
...
-   Work through [data collection examples](https://stats.ifp.uni-mainz.de/ma-ccs/content-data.html)
...
-   formal specification with probabilistic components
...
-   LLMs supplement verbal instructions and role with verbal responses
-   responses depend on training data and LLM quality
-   very few validation studies to date [@larooij2025]
-   often relatively naive assumptions about the generalizability of results (i.e., treating LLMs as substitutes for human agents)
...
-   generating stimulus × persona combinations
