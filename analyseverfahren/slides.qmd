...
-   Übungsmaterialien samt Daten und R-Code auf <br/> <https://stats.ifp.uni-mainz.de/ba-aa-vl>
...
-   Verfahren für die statistische **Punkt**- und **Intervallschätzung**
-   basieren auf bestimmten Annahmen über die Stichprobe und Variable(n)
...
-   wir simulieren die Körpergröße in der Grundgesamtheit von N = 1200 Studierenden am IfP
-    wir prüfen, wie gut unsere Schätzung der Körpergröße durch eine einzelne Stichprobe gelingt
:::
...
-   blau: Mittelwert in der Grundgesamtheit
-   rot: Mittelwert in der einzelnen Stichprobe
:::
...
:::::: columns
::: {.column width="40%"}
-   die Mittelwerte der einzelnen Stichproben streuen um den wahren Populationsmittelwert von `r round(mean(GG))` mit einem Standardfehler (SE)
...
-   bei den 1000 simulierten Stichproben ist der Mittelwert der Mittelwerte M = `r round(mean(STPx1000.mean), 1)`
-    Standardabweichung der Mittelwerte: SE = `r round(sd(STPx1000.mean),1)`
:::
::::::
...
-   dieses Wissen können wir mit den Schätzern aus *einer* Stichprobe kombinieren
...
-   95%-Konfidenzintervall: in 95 Prozent aller denkbaren Stichproben würde das Konfidenzintervall den wahren Populationswert enthalten
...
-   im Gegensatz zu den unendlich vielen Alternativhypothesen, die man haben kann, gibt es immer nur **eine Nullhypothese**
    -   "Es besteht kein Unterschied/Abhängigkeit zwischen Gruppen oder Variablen."
-   von fast allen Verfahren (T-Test, Korrelation, Chi-Quadrat-Test, etc.) wissen wir, wie die "Welt der Nullhypothese" aussieht
-   Prüfung der Stichprobenkennwerte daraufhin, wie häufig oder wahrscheinlich sie in der Welt der Nullhypothese sind
-   diese Häufigkeit/Wahrscheinlichkeit ist der p-Wert
...
-   **p(Daten\|H1)**: Die Wahrscheinlichkeit, die empirischen Daten zu beobachten, wenn die Alternativhypothese gilt
...
-   Ergebnis ist statistisch signifikant, wenn $p < \alpha$, wobei $\alpha$ das zuvor angenommene Signifikanzniveau ist
-   zwei mögliche Ursachen:
    -   Nullhypothese gilt, aber wir beobachten zufällig ein sehr seltenes Ereignis *oder*
    -   Nullhypothese gilt nicht. (Das heißt **nicht**, die Alternativhypothese gilt.)
...
:::::: columns
::: {.column width="40%"}
-   $\alpha = .05$ bedeutet, dass wir in 5 von 100 Fällen ein signifikantes Ergebnis bekommen, obwohl die Nullhypothese gilt
-   mit unseren 1000 Stichproben und $\mu_0 = 170$ sollten wir ca. 50 fälschlich signifikante Ergebnisse bekommen
-   Alpha-Fehler ist abhängig von $\alpha$, aber unabhängig von den Daten und der konkreten $H_0$
:::
...
-   sog. **p-Hacking** bedeutet, die Daten und Analysen so zu frisieren, bis ein stat. signifikantes Ergebnis erscheint
...
-   $b_1$ gibt (bei Dummy-Codierung) die Differenz zwischen der Gruppe 1 und der Referenzgruppe wieder, $b_2$ die Differenz zwischen der Gruppe 2 und der Referenzgruppe, etc.
...
-   p-Werte (manchmal auch die CI) sollten daher korrigiert werden (vgl. Bender & Lange, 2001)
...
-   globaler F-Test als Modellvergleich: Residualvarianz meines Modells vs. Nullmodell
...
-   Beispiele:
    -   Experimente mit min. 2 Faktoren, die sich gegenseitig beeinflussen
...
```{r}
lm(Information_overload ~ Gender * Education, data = vanerkel21) |> 
  model_table()
```
...
```{r}
lm(gesamt ~ schwab + atol, data = vb17) |> 
  model_table()
```
...
```{r}
lm(gesamt ~ schwab * I(atol - mean(atol, na.rm = T)), data = vb17) |> # atol = 1
  model_table()
```
...
3.  Gibt es den im Titel des Artikels angesprochenen Effekt eines **zusätzlichen "interesting-if-true"** Effekts auf das Teilen von Nachrichten? Wie fällt dieser Interaktionseffekt aus?
...
1.  Stellen Sie die Ergebnisse des/der geschätzten Modells/Modelle sinnvoll tabellarisch oder grafisch dar (Screenshot reicht).
...
3.  Lösung als **PDF-Datei** bitte bis 09.07.2025, 10 Uhr in Moodle hochladen.
...
x <- 18:80
y <- invlogit(-30 + .5 * x) + rnorm(63, sd = .3)
vm <- qplot(x, round(jitter(y))) + ylim(0, 1) + xlim(30, 80) + labs(x = "Alter", y = "Person mag Volksmusik")
#vm
```
...
vm + geom_smooth(method = "lm", width = 2, se = F) + ylim(-.5, 1.5)
```
...
vm + geom_smooth(method = "glm", method.args = list(family = "binomial"), se = F, width = 2)
```
...
![](images/festl13.png){fig-align="center"}
...
model_table(m1_gender)
```

## Odds-Ratios (Exp(B))

```{r, echo = F}
report::report_table(m1_gender, exponentiate = TRUE, include_effectsize = FALSE) |> 
  slice(1:2) |> 
...
marginaleffects::avg_comparisons(m1_gender) |>
  as_tibble() |> 
select(-contrast, -s.value, -starts_with("pred")) 
```
...
model_table(m2_gender_internet)
```
...
library(performance)
library(tidyverse)
uni_fb <- haven::read_sav("data/Faehnrich_2020.sav") |> mutate(uni_fans = uni_fans / 10)
...
```{r}
lm(Share ~ Type + Interesting + Type:Interesting, data = altay22) |> 
  marginaleffects::avg_predictions(variables = c("Type", "Interesting")) |> 
  filter(Interesting %in% c(1,4,7)) |> 
  ggplot(aes(x = Interesting, y = estimate, ymin = conf.low, ymax = conf.high, color = Type, group = Type))+
  geom_pointrange(position = position_dodge(.5))+
  geom_line(position = position_dodge(.5))+
  scale_color_discrete(labels = c("False News", "True News"))+
...
* Schüler*innen geschachtelt in Klassen oder Schulen
...
- Daten können auch auf mehr als zwei hierarchischen Ebenen (Schüler*innen, Klasse, Schule) geschachtelt oder kreuzklassifiziert sein
...
![](images/gelman_hill.png){fig-align="center"}
\
...
![](images/faehnrich.png)

## Daten

- Stichprobe von `r nrow(uni_fb)` Facebook-Posts von `r n_distinct(uni_fb$uni)` Universitäten 
...
faehnrich19 |> 
select(uni, uni_fans, created_time, type, likes_count, comments_count) |> 
sample_n(5)
```
...
model_table(m1_research, fixed = T)
```

## Modellvorhersagen (varying intercepts)

```{r}
preds_uni <- marginaleffects::avg_predictions(m1_research, variables = c("topic_research", "uni")) |>
  as_tibble()
preds_uni |>
  ggplot(aes(x = factor(topic_research), y = estimate, color = uni, group = uni)) +
  geom_line(show.legend = FALSE) +
  geom_point(show.legend = FALSE) +
  labs(x = "Topic Research", y = "Predicted # of comments")
```
...
```{r}
preds_uni_vs <- marginaleffects::avg_predictions(m2_research_vs, variables = c("topic_research", "uni")) |>
  as_tibble()
```
...
model_table(m3_research_fans, fixed = T)
```
...
- in den Studien von Altay et al. (2022) haben alle TeilnehmerInnen *mehrere* Artikel bewertet
...
```{r}
lmer(Share ~ 1 + (1|Participants), data = altay22) |> 
  performance::icc() 
```
...
```{r}
altay22$Interesting_c = scale(altay22$Interesting, scale = F)
m_multi = lmer(Share ~ Type * Interesting_c + (1|Participants), data = altay22)
m_flat = lm(Share ~ Type * Interesting_c, data = altay22)
sjPlot::tab_model(m_flat, m_multi)
```
