...
topics = "Einführung und Grundlagen, Themenwahl und Forschungsfragen, CCS: Datenerhebung, Studiendesign und Stichprobe*, Kategorien und Codierung*, CCS: Inhaltsanalyse mit LLM, Codebuch-Finalisierung*, Reliabilitätstest, Feldphase (keine Sitzung),  Datenaufbereitung und -analyse*,  CCS: Zero-Shot Codierung I, CCS: Zero-Shot Codierung II, Visualisierung und Ergebnisaufbereitung, Abschluss" |> str_split_1(", ") |> str_squish() 
make_schedule(start_date, end_date, no_classes, topics) |> 
  mutate(Datum = strftime(Datum, "%d.%m.%Y")) |> kableExtra::kable(format.args = )
```
:::

## Literatur

![](https://images-na.ssl-images-amazon.com/images/I/51+sDU+LbxL._SX347_BO1,204,203,200_.jpg){height="600px" align="right"} ![](https://media.springernature.com/full/springer-static/cover-hires/book/978-3-658-36179-2?as=webp){height="600px" align="right"} ![](https://pictures.abebooks.com/isbn/9781506395661-us.jpg){height="600px" align="right"}
...
-   Hausaufgaben für jede CCS-Sitzung, Abgabe jeweils Mo, 12h in Moodle
...
1.  Arbeiten Sie <https://stats.ifp.uni-mainz.de/ba-ccs-track/ia-daten.html> einmal durch
2.  Hausaufgabe ist in der übernächsten Woche fällig (spätestens, nicht frühestens!)
...
![](https://medien.umbreitkatalog.de/bildzentrale_original/978/365/840/1702.jpg){height="600px"} ![](https://media.springernature.com/full/springer-static/cover-hires/book/978-3-658-37747-2?as=png){height="600px"} ![](https://media.wiley.com/product_data/coverImage300/39/11196802/1119680239.jpg){height="600px"}
...
2.  Finalisieren Sie semantische und dimensionale Analyse
3.  Melden Sie Gruppenmitglieder und Zuständigkeiten
...
-   Beispiel:
    ...
    2.  Auswahl von 100 zufälligen Beiträgen aus 2025 (einfache Zufallsstichprobe)
...
1.  Finalisieren Sie die Forschungsfragen und Hypothesen.
2.  Diskutieren und finalisieren Sie das Untersuchungsdesign Ihrer Studie.
3.  Definieren Sie Untersuchungseinheit(en) und Grundgesamtheit.
4.  Entwickeln Sie einen konzeptionell angemessenen Stichprobenplan.
5.  Überlegen Sie, wie die Stichprobenziehung praktisch umgesetzt werden kann.
...
![Quelle: @tornberg2023 ](images/clipboard-970418601.png)
...
1.  Erledigen Sie die Hausaufgabe auf <https://stats.ifp.uni-mainz.de/ba-ccs-track/ia-zeroshot.html>
2.  Codieren Sie in der Gruppe 20 Codiereinheiten für einen Reliabilitätstest, d.h. einzeln (!) und jede/r dieselben (!) Fälle. Fügen Sie die Codierungen in ein Codesheet untereinander (wichtige Variablen: coder_id und unit_id).
...
```{r, echo = T}
my_model = lm(hp ~ factor(cyl), data = mtcars)
report::report_table(my_model)
```
...
```{r}
marginaleffects::avg_predictions(my_model, by = "cyl")
```


## Visualisierung I: deskriptiv

```{r, echo = T}
mtcars |> group_by(cyl) |> summarise(mean_hp = mean(hp)) |> 
  ggplot(aes(x = factor(cyl), y = mean_hp))+ # cyl als kategoriell
  geom_col() + labs(x = "Zylinder", y = "Leistung in PS", caption = "n = 32")
```
...
```{r, echo = T}
marginaleffects::avg_predictions(my_model, by = "cyl") |> 
  ggplot(aes(x = factor(cyl), y = estimate, ymin = conf.low, ymax = conf.high))+
  geom_pointrange()+theme_minimal()+
  labs(x = "Zylinder", y = "Leistung in PS", caption = "n = 32")
```
...
- Darstellung von statistischer Unsicherheit über Konfidenzintervalle
